  ...
  <div style="margin-top:60px" class="p-5">
    ...
    >
      ...
      >
        ...
        >
          <div
            class="w-full inset-y-0 left-0 bg-gray-200 max-w-sm rounded-lg overflow-hidden shadow-lg"
            style="height: 570px"
          >
            <div
  ...
  if (
    ...
    store.dispatch("hookers/checkHookRequest", uid).then((res) => {
      if (res.data.length > 0) {
        ...
        store.dispatch("hookers/sendHookRequest", { uid, name }).then(() => {
          store.commit("notify", {
            type: "success",
