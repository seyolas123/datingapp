<template>
    ...
    >
    <p class="mt-3 max-w-2xl text-sm text-gray-500"><em>user suggestions for you</em></p>
      <swiper
      ...
      >
        ...
        >
          <div
            ...
            style="height: 570px"
          >
            <div class="px-2 py-3 flex justify-center items-center" v-if="user.avatar">
            
              <CloudImageProvider v-bind:cloudImageConfig="cloudimageConfig">
                    
                    <img class="w-full rounded-lg shadow-lg" style="width:400px; height:400px;" 
                    :src="config.urlink + user.avatar" :alt="user.name" />
                    </CloudImageProvider>
            </div>
  ...
  const matches = computed(() => store.state.users.match.data);

  store.dispatch("users/getUserMatch");
